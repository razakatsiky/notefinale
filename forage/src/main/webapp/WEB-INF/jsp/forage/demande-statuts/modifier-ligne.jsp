...
<jsp:include page="../template.jsp">
    <jsp:param name="title" value="Modifier la ligne"/>
</jsp:include>

<div class="container" style="margin-top: -70px !important;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px;">
        <div>
            <h2 style="font-size: 1.75rem; font-weight: 600; color: black;">Modifier la ligne de statut - Demande #${demande.id}</h2>
    ...
    <div style="background: white; border: 1px solid rgba(0, 0, 0, 0.1); border-radius: 8px; padding: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
            <h3 style="font-size: 1.25rem; font-weight: 600; margin: 0;">Modifier l'observation et la date</h3>
        </div>
        
        <form action="${pageContext.request.contextPath}/demande-statuts/modifier-ligne/${demande.id}/${statutAModifier.id}" method="post">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                <div>
                    <label style="display: block; margin-bottom: 8px; font-weight: 500; color: black;">Statut *</label>
                    <select name="statutNouveauId" required style="width: 100%; padding: 12px 16px; border: 1px solid rgba(0, 0, 0, 0.2); border-radius: 4px; font-size: 14px; background: white; color: black;">
                        <option value="">-- Sélectionner un statut --</option>
                        <c:forEach var="statut" items="${statuts}">
                            <option value="${statut.id}" ${statutAModifier.statut.id == statut.id ? 'selected' : ''}>${statut.libelle}</option>
                        </c:forEach>
                    </select>
                </div>
                <div>
                    <label style="display: block; margin-bottom: 8px; font-weight: 500; color: black;">Date du statut *</label>
                    <c:set var="dateValue" value="${statutAModifier.dateStatut}"/>
                    <input type="datetime-local" name="dateStatut" required 
                           value="${dateValue.year}-${dateValue.monthValue < 10 ? '0' : ''}${dateValue.monthValue}-${dateValue.dayOfMonth < 10 ? '0' : ''}${dateValue.dayOfMonth}T${dateValue.hour < 10 ? '0' : ''}${dateValue.hour}:${dateValue.minute < 10 ? '0' : ''}${dateValue.minute}"
            ...
            <div style="margin-bottom: 16px;">
                <label style="display: block; margin-bottom: 8px; font-weight: 500; color: black;">Observation</label>
                <textarea name="observation" rows="4" placeholder="Entrez une observation pour cette ligne de statut..." 
                          style="width: 100%; padding: 12px 16px; border: 1px solid rgba(0, 0, 0, 0.2); border-radius: 4px; font-size: 14px; background: white; color: black; resize: vertical; line-height: 1.5;">${statutAModifier.observation}</textarea>
            </div>
            
            <div style="display: flex; gap: 12px;">
                <button type="submit" style="background: #3498db; color: white; padding: 12px 24px; border: none; border-radius: 4px; cursor: pointer; font-weight: 500;">Modifier ligne</button>
