...
<jsp:include page="../template.jsp">
    <jsp:param name="title" value="Mettre à jour le statut"/>
</jsp:include>

<div class="container" style="margin-top: -70px !important;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px;">
        <div>
            <h2 style="font-size: 1.75rem; font-weight: 600; color: black;">Mettre à jour le statut de la demande #${demande.id}</h2>
    ...
    <div style="background: white; border: 1px solid rgba(0, 0, 0, 0.1); border-radius: 8px; padding: 24px; margin-bottom: 24px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            ...
            <div>
                ...
                <!-- <c:if test="${not empty demande.latestStatut}">
    ...
    <div style="background: white; border: 1px solid rgba(0, 0, 0, 0.1); border-radius: 8px; padding: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
            <h3 style="font-size: 1.25rem; font-weight: 600; margin: 0;">Nouveau statut</h3>
        </div>
        
        <form action="${pageContext.request.contextPath}/demande-statuts/mettre-a-jour/${demande.id}" method="post">
                ...
                <div>
                    ...
                    <select name="statutId" required style="width: 100%; padding: 12px 16px; border: 1px solid rgba(0, 0, 0, 0.2); border-radius: 4px; font-size: 14px; background: white; color: black;">
                        ...
                        <c:forEach var="statut" items="${statuts}">
                            <option value="${statut.id}" ${demande.latestStatut.statut.id == statut.id ? 'selected' : ''}>${statut.libelle}</option>
                        </c:forEach>
                    </select>
                </div>
                <div>
                    ...
                    <c:if test="${not empty demande.latestStatut}">
                        <c:set var="dateValue" value="${demande.latestStatut.dateStatut}"/>
                    ...
                    <c:if test="${empty demande.latestStatut}">
                        <input type="datetime-local" name="dateStatut" required 
                               style="width: 100%; padding: 12px 16px; border: 1px solid rgba(0, 0, 0, 0.2); border-radius: 4px; font-size: 14px; background: white; color: black;">
                    </c:if>
                </div>
            ...
            <div style="margin-bottom: 16px;">
                ...
                <textarea name="observation" rows="4" placeholder="Entrez une observation pour ce changement de statut..." 
                          style="width: 100%; padding: 12px 16px; border: 1px solid rgba(0, 0, 0, 0.2); border-radius: 4px; font-size: 14px; background: white; color: black; resize: vertical; line-height: 1.5;">${demande.latestStatut.observation}</textarea>
            </div>
            
            <div style="display: flex; gap: 12px;">
                <button type="submit" style="background: black; color: white; padding: 12px 24px; border: none; border-radius: 4px; cursor: pointer; font-weight: 500;">Mettre à jour</button>
