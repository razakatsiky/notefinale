...
<head>
    ...
    <title>Calculer Note Finale</title>
    <style>
            ...
            <div class="form-group">
                ...
                <select name="candidatId" id="candidatId" required>
                    ...
                    <c:forEach items="${candidats}" var="candidat">
                        <option value="${candidat.id}">ID: ${candidat.id} - ${candidat.nom}</option>
                    </c:forEach>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="matiereId" id="matiereId" required>
                    ...
                    <c:forEach items="${matieres}" var="matiere">
                        <option value="${matiere.id}">ID: ${matiere.id} - ${matiere.nom}</option>
                    </c:forEach>
                </select>
            </div>
            
            <button type="submit" class="btn">Calculer la Note Finale</button>
        </form>
        
        <c:if test="${success}">
            <div class="success">
                <strong>Calcul effectué avec succès!</strong>
            </div>
            
            <div class="result">
                <h3>Résultat pour ${candidat.nom} - ${matiere.nom}</h3>
                ...
                <c:if test="${not empty notes}">
                    <div class="notes-list">
                        <h4>Notes utilisées:</h4>
                        <c:forEach items="${notes}" var="note">
                            <div class="note-item">
                                Note: ${note.note} (Correcteur: ${note.correcteur.nom})
                            </div>
                        </c:forEach>
                    </div>
                </c:if>
